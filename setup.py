...
if sys.version_info[:2] < (3, 8):
    print(
        "SymEngine requires Python 3.8 or newer. "
...
if use_distutils is not None:
    if use_distutils.lower() == "true":
        ...
        print("Value {} for USE_DISTUTILS treated as False".format(use_distutils))
...
global_user_options = [
    ...
    ("define=", "D", "options to cmake <var>:<type>=<value>"),
]
...
def _process_define(arg):
    ...
    return [
        (s.strip(), None) if "=" not in s else tuple(ss.strip() for ss in s.split("="))
        for s in defs
    ]
...
class BuildExtWithCmake(_build_ext):
    ...
    def cmake_build(self):
        ...
        if (
            subprocess.call(
                ["cmake", "--build", ".", "--config", cmake_build_type[0]],
                cwd=build_dir,
            )
            != 0
        ):
...
class InstallWithCmake(_install):
    ...
    def finalize_options(self):
        ...
        cmake_build_type[0] = self.build_type
        cmake_opts.extend(
            [("PYTHON_INSTALL_PATH", path.join(os.getcwd(), self.install_platlib))]
        )
